...
async function main(): Promise<void> {
  ...
  const scenePath = values.scene ?? process.env.PASCAL_SCENE_FILE
  if (scenePath) {
    ...
    if (sceneId) {
      bridge.setActiveScene({
        id: sceneId,
        name: process.env.PASCAL_SCENE_NAME ?? 'Active Scene',
        projectId: process.env.PASCAL_PROJECT_ID ?? sceneId,
        ownerId: null,
        thumbnailUrl: null,
        version: Number(process.env.PASCAL_SCENE_VERSION ?? '1'),
      })
    }
  } else {
    bridge.loadDefault()
